...
try:
    ...
    print("⚠️  RAG not available — install langchain stack")
...
# Shared RAG paths — same as bayazid.py
...
def load_pdf_smart(pdf_path: str) -> list:
    try:
        ...
        if sum(len(p.page_content.strip()) for p in pages) > 100:
            return pages
    except Exception:
        pass
    if OCR_AVAILABLE:
...
def _build_or_load_rag():
    """Load or incrementally build the shared FAISS index."""
    global _rag_retriever, _RAG_DISABLED
    ...
    try:
        ...
        already_indexed = set(
            json.load(open(index_tracker)) if os.path.exists(index_tracker) else []
        )
        ...
        all_pdfs = (
            {fn for fn in os.listdir(DOC_DIR) if fn.lower().endswith(".pdf")}
            if os.path.exists(DOC_DIR) else set()
        )
        new_pdfs = sorted(all_pdfs - already_indexed)

        if new_pdfs:
            splitter = RecursiveCharacterTextSplitter(chunk_size=450, chunk_overlap=40)
            for pdf_file in new_pdfs:
                print(f"  [RAG] Indexing: {pdf_file}")
                ...
                valid  = [
                    ...
                    and len(c.page_content.strip()) > 10
                    and any(ch.isalnum() for ch in c.page_content)
                ]
                if valid:
                    if vectorstore is None:
                        vectorstore = FAISS.from_documents(valid, embed)
                    else:
                        try:
                            vectorstore.add_documents(valid)
                        except Exception as e:
                            print(f"  [!] Chunk embed error: {e}")
                    vectorstore.save_local(FAISS_DIR)
                already_indexed.add(pdf_file)
                with open(index_tracker, "w") as f:
                    json.dump(list(already_indexed), f)
                del pages, chunks, valid
                gc.collect()
...
def get_rag_context(query: str) -> str:
    """Retrieve and format relevant book context for a query."""
    global _rag_retriever, _RAG_DISABLED
    if _RAG_DISABLED:
        return ""
